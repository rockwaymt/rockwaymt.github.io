...
///////////////preloader ///////////////////////////
////////////////////////////////////////////////////////
function loadDiv(divID, page) {
	var xhr= new XMLHttpRequest();
	xhr.onreadystatechange= function() {
		if (this.readyState!==4) return;
		var right = document.getElementById(divID);
		if(right !== null) {
			right.innerHTML= this.responseText;
		}
	};
	if(!page) { page = "sections/"+divID+".html"; }
	//console.log("Loading:", divID, page);
	document.getElementById(divID).removeAttribute("data-hide");
	document.getElementById("m"+divID).removeAttribute("data-hide");
	xhr.open('GET', page, true);
	xhr.send();
}
function dynAdd() {
	var pl = document.getElementsByTagName("body")[0], i, x;
	// "about", "contact"   // Always enabled
	//console.log("Checking...");
	if(!pl || pl.style.overflow != "visible") { 
		setTimeout(dynAdd, 200); 
		return;
	}
	if(!sections || sections.length == 0) { console.log("No dynamic sections");return;}
	for(i = 0; i < sections.length; i++) {
		loadDiv(sections[i][0], sections[i].length > 1 ? sections[i][1] : null);
	}
}
...
//            animation: google.maps.Animation.BOUNCE,
	...
	var contentString = "<p class='map-info'><strong>Rockway Massage Therapy</strong> <br/> 589 Charles Street East, Kitchener <br/> (519) 744-4745</p>";	// HTML text to display in the InfoWindow
